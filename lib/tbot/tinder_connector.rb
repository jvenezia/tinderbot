module Tbot
  class TinderConnector
    ...
    CONNECTION_USER_AGENT = 'User-Agent: Tinder/3.0.4 (iPhone; iOS 7.1; Scale/2.00)'
    ...
    def self.get_tinder_connexion
      ...
      connection.tap { |c| c.headers[:user_agent] = CONNECTION_USER_AGENT }
    end

    def self.sign_in(connexion, facebook_authentication_token, facebook_user_id)
      authentication_response = JSON.parse(connexion.post('/auth', {facebook_token: facebook_authentication_token, facebook_id: facebook_user_id}).body)
      tinder_authentication_token = authentication_response['token']
      connexion.token_auth(tinder_authentication_token)
      connexion.headers['X-Auth-Token'] = tinder_authentication_token
    end
